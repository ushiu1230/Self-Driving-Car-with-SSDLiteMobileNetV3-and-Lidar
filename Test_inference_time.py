...
import_module = 'from detect_utils import predict'
...
def object_detection():
    while True:
        ...
        with torch.no_grad():
            testcode = 'boxes, classes, labels = predict(frame, model, device, 0.9)'
            t0 = timeit.Timer(stmt= testcode, setup= import_module, globals= {'frame':frame, 'model':model,'device':device})
            print(f'time: {t0.timeit(100) / 100 * 1e6:>5.1f} us')
            break
