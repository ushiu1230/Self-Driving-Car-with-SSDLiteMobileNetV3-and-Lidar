...
from torchvision.models.detection.ssdlite import SSDLiteRegressionHead, SSDLiteClassificationHead
...
def Model2(num_classes):
    ...
    in_channels = det_utils.retrieve_out_channels(model.backbone, (320, 320))
    model.anchor_generator = DefaultBoxGenerator([[2,3],[2,3],[2,3],[2,3]], min_ratio=0.2, max_ratio=0.9)
    num_anchors = model.anchor_generator.num_anchors_per_location()
...
def Model4(num_classes):
  ...
  model.backbone = backbone
  in_channels = det_utils.retrieve_out_channels(model.backbone, (320, 320))
  #model.anchor_generator = DefaultBoxGenerator(aspect_ratios = [[1.0, 2.0],[1.0, 2.0],[1.0, 2.0],[1.0, 2.0]])
  num_anchors = model.anchor_generator.num_anchors_per_location()
  #aspect_ratios = model.anchor_generator.aspect_ratios
  norm_layer  = partial(nn.BatchNorm2d, eps=0.001, momentum=0.03)
  model.head.regression_head = SSDLiteRegressionHead(in_channels, num_anchors, norm_layer)
